...
AS $$
  SELECT
    ...
    ti.webhook_url,
    ti.updated_at,
    ti.api_key_secret_id IS NOT NULL AS has_api_key,
    ti.webhook_secret_secret_id IS NOT NULL AS has_webhook_secret
...
AS $$
  SELECT
    ...
    ti.tenant_id,
    ti.provider,
    api_secret.decrypted_secret AS api_key,
    webhook_secret.decrypted_secret AS webhook_secret,
    ti.webhook_url,
    ti.instance_name,
    ti.status,
...
BEGIN
  ...
  ELSIF v_api_key_trimmed IS NOT NULL THEN
    ...
    UPDATE public.tenant_integrations
    SET api_key_secret_id = v_api_key_secret_id,
        updated_at = now()
    WHERE id = v_row.id
    RETURNING * INTO v_row;
  END IF;
  ...
  IF p_clear_webhook_secret THEN
    UPDATE public.tenant_integrations
    SET webhook_secret_secret_id = NULL,
        updated_at = now()
    WHERE id = v_row.id
    RETURNING * INTO v_row;
  ELSIF v_webhook_secret_trimmed IS NOT NULL THEN
    ...
    UPDATE public.tenant_integrations
    SET webhook_secret_secret_id = v_webhook_secret_secret_id,
        updated_at = now()
    WHERE id = v_row.id
    RETURNING * INTO v_row;
  END IF;
