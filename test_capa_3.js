...
async function act() {
    try {
        ...
        for (const p of processos) {
            ...
            // 1. CAPA
            const capaRes = await fetch(`https://api.escavador.com/api/v2/processos/numero_cnj/${num}/capa`, {
                method: 'GET',
                headers: {
            ...
            console.log(`[CAPA] HTTP ${capaRes.status}`);
            const capaJ = await capaRes.json();
            if (capaRes.status === 200) {
                console.log(JSON.stringify(capaJ, null, 2));
            }

            // 2. RESUMO-IA
            const iaRes = await fetch(`https://api.escavador.com/api/v2/processos/numero_cnj/${num}/resumo-ia`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${escavadorKey}`,
                    'X-Requested-With': 'XMLHttpRequest'
            ...
            console.log(`[RESUMO-IA] HTTP ${iaRes.status}`);
            const iaJ = await iaRes.json();
            if (iaRes.status === 200) {
                console.log(JSON.stringify(iaJ, null, 2));
            }
        }
