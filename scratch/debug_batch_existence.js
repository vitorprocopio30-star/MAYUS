...
async function run() {
  ...
  for (const P of PROCESSOS) {
    ...
    // Check Capa
    try {
      const respCapa = await fetch(`${BASE}/processos/numero_cnj/${P}`, { headers });
      if (respCapa.ok) {
        const data = await respCapa.json();
        console.log(`  [CAPA]: OK - ${data.titulo || 'No title'}`);
      } else {
        console.log(`  [CAPA]: FAILED (${respCapa.status})`);
      }
    } catch (e) {
      console.log(`  [CAPA]: ERROR - ${e.message}`);
    }

    // Check AI Summary
    try {
      const respAI = await fetch(`${BASE}/processos/numero_cnj/${P}/ia/resumo`, { headers });
      if (respAI.ok) {
        const data = await respAI.json();
        console.log(`  [AI]: OK`);
        console.log(`  Summary: ${data.resumo ? data.resumo.substring(0, 100) + '...' : 'Empty'}`);
      } else {
        console.log(`  [AI]: FAILED (${respAI.status})`);
      }
    } catch (e) {
