...
async function act() {
    try {
        ...
        const procResp = await fetch(SUPABASE_URL + '/rest/v1/monitored_processes?select=numero_processo&tenant_id=eq.' + TENANT_ID + '&limit=3', {
            headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + SUPABASE_KEY }
        });
        const processos = await procResp.json();
        
        if (!processos || !processos.length) return console.log('Nenhum processo encontrado');

        for (const p of processos) {
            const num = p.numero_processo;
            console.log(`\n\n=== Buscando capa para: ${num} ===`);
