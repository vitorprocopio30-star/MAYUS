...
- [x] Prontidao agentica do escritorio adicionada em 2026-05-03: `runTenantDoctor` agora devolve score, nivel e proximo melhor passo, grava esses sinais no artifact/evento do Setup Doctor e Configuracoes exibe a acao prioritaria supervisionada.
- [x] Modo Beta MAYUS iniciado em 2026-05-03: `POST /api/setup/beta` cria uma missao/artifact `tenant_beta_workplan` com fila operacional supervisionada para Setup, Playbook Diario, CRM sem proximo passo e status de caso, mantendo side effects externos bloqueados.
- [x] Fila beta virou missao acompanhavel em 2026-05-03: cada item do `tenant_beta_workplan` cria `brain_steps`; itens seguros ficam `queued`, itens sensiveis ficam `awaiting_approval` e criam `brain_approvals` pendentes.
- [x] Executor do proximo item beta entregue em 2026-05-03: `POST /api/setup/beta/execute-next` conclui o proximo step seguro da fila, grava artifact `tenant_beta_step_result`, learning event `tenant_beta_step_completed` e atualiza Configuracoes.
- [x] Primeiro handler real da fila beta conectado em 2026-05-03: step `core:daily_playbook` agora gera Playbook Diario com dados de CRM/agenda/configuracoes, salva artifact `daily_playbook` dentro da missao beta e preserva bloqueio de envio externo.
- [x] Handler Growth da fila beta conectado em 2026-05-03: step `growth:crm_next_steps` usa o motor `marketing_ops_assistant` para organizar leads sem proximo passo, salva artifact `marketing_ops_assistant_plan` e mantem side effects externos bloqueados.
- [x] Handler Lex da fila beta conectado em 2026-05-03: step `lex:support_case_status` usa contrato/reply seguro de status do caso, salva artifact `support_case_status` e mantem handoff humano quando a base for insuficiente.
- [x] Status da missao beta passou a acompanhar a fila em 2026-05-03: apos cada `execute-next`, `brain_tasks` e `brain_runs` viram `executing`, `awaiting_approval` ou `completed` conforme steps restantes e aprovacoes pendentes.
- [x] Execucao continua da fila segura beta entregue em 2026-05-03: `POST /api/setup/beta/execute-safe-queue` roda itens seguros em sequencia, para em aprovacao humana/conclusao/limite e retorna resumo operacional.
...
- [x] Atendimento consultivo no WhatsApp iniciado em 2026-04-28 e promovido em 2026-05-03 para operacao por padrao: `whatsapp-sales-reply` e `whatsapp-agent-runtime` respondem venda, pedido de reuniao/agendamento e suporte quando o modo do escritorio nao for `human_only`; `human_only` virou a configuracao explicita de "Somente Rascunho".
- [x] Autoatendimento inbound do WhatsApp iniciado em 2026-04-28 e ajustado em 2026-05-03: Meta Cloud e Evolution chamam `prepareWhatsAppMayusReplyForContact` apos salvar mensagem recebida; o MAYUS pode autoenviar primeira resposta consultiva/suporte com typing/presence quando ha integracao, registrar auditoria e manter rascunho apenas se o usuario escolher esse modo.
- [x] Visibilidade operacional do MAYUS no WhatsApp adicionada em 2026-04-28: `GET /api/whatsapp/ai-sales-reply` busca o ultimo evento do contato e a tela de conversa exibe painel para usar/atualizar o texto quando o escritorio operar em modo rascunho ou quando o envio automatico falhar.
- [x] WhatsApp Command Center promovido em 2026-05-03 para operador interno: telefone autorizado pode pedir relatorio/playbook, CRM, agenda ou status juridico; o MAYUS gera saudacao por horario, resumo executivo, link de Playbook HTML premium e responde sem inventar cliente quando falta nome/telefone/CNJ.
- [x] Skill comercial Dutra isolada por tenant entregue em 2026-05-03: `commercial-playbook-template` agora resolve a Metodologia Blindagem/RMC/GRAM apenas para contexto Dutra, cria fallback generico sem termos proprietarios para demais escritorios, inclui questionario de configuracao e conecta WhatsApp/Playbook Diario ao playbook resolvido. Validacoes passaram: 4 arquivos, 21 testes focados e typecheck.
- [x] Auto-organizacao agentica do MAYUS iniciada em 2026-05-03: `organizeTenantAutonomously` continua fila segura existente, evita duplicar ciclo quando aguarda aprovacao e inicia novo ciclo quando necessario; `POST /api/setup/organize`, chat e cockpit conseguem disparar a organizacao segura do escritorio. A UI do cockpit trocou linguagem de "missao" por "pensando", "pesquisando", "organizando" e "atividade do MAYUS". Validacoes passaram: 5 arquivos, 36 testes focados e typecheck.
...
- [x] Padrao `Executar` iniciado em 2026-05-02: Monitoramento exibe organizacao por IA como execucao de fase e Prazos ganhou `POST /api/prazos/executar-agenda` mais botao `Executar na agenda`, distribuindo prazos filtrados em `user_tasks` via sincronizacao idempotente e registrando `system_event_logs.prazos_execute_agenda`. Validacoes passaram: 16 testes focados, typecheck e diff-check.
- [x] WhatsApp demo entrou no padrao `Executar` em 2026-05-02: `/dashboard/conversas/whatsapp` mostra `Executar` no rascunho MAYUS de contatos seed da conta modelo; `POST /api/whatsapp/execute-reply` registra mensagem outbound simulada, atualiza contato e audita `whatsapp_demo_reply_executed`, bloqueando contato real com envio humano supervisionado. Validacoes passaram: 9 testes focados e typecheck.
- [x] Drive demo alinhado ao fluxo real em 2026-05-02: reset da conta modelo nao grava mais `drive_folder_id`/links fake; o botao de documentos virou `Executar Acervo` e deve usar a integracao Google Drive normal quando uma conta Google exclusiva da demo for conectada. Validacoes passaram: typecheck e 10 testes focados.
- [x] Painel super admin da conta modelo iniciado em 2026-05-02: `/admin/demo` lista tenants, marca/desmarca `demo_mode`, mostra modos de Drive/WhatsApp/Escavador da demo, executa dry-run/reset real via `/api/admin/demo/reset` e mostra prontidao do Drive dedicado sem expor tokens ou `api_key`; nova rota `/api/admin/demo/status` preserva `tenant_settings.ai_features` existentes e audita `demo_tenant_status_updated`. Validacoes passaram: 12 testes focados e typecheck.
- [x] Banner visual permanente da conta demo entregue em 2026-05-02: `DemoEnvironmentBanner` foi plugado no `DashboardLayout` e usa `isDemoModeEnabled` para exibir ambiente de demonstracao, dados sinteticos e modos demo em todas as telas do dashboard. Validacoes passaram: 18 testes focados e typecheck.
- [~] Suporte MAYUS amplo iniciado em 2026-05-02: `/api/admin/support/tenants` e `/admin/support` mostram tenants com resumo sanitizado de usuarios, integracoes, demo mode e necessidade de grant para dado sensivel; nenhum token, customer id ou metadata de integracao e retornado. Validacoes passaram: 7 testes focados e typecheck.
- [x] Grants temporarios de suporte MAYUS entregues em 2026-05-02: migration `admin_support_grants`, `POST /api/admin/support/grants`, revoke por `POST /api/admin/support/grants/:id/revoke` e UI em `/admin/support` criam/revogam acesso temporario com motivo, escopo, expiracao e auditoria em `system_event_logs`. Validacoes passaram: 6 testes focados e typecheck.
- [x] Primeira visualizacao protegida por grant entregue em 2026-05-02: `getActiveSupportGrant` valida escopo/validade e `GET /api/admin/support/tenants/:id/sensitive-summary` exige `tenant_sensitive_readonly`, retorna apenas resumo redigido e audita `support_access_viewed`. Validacoes passaram: 11 testes focados e typecheck.
- [~] Inbox de suporte MAYUS iniciada em 2026-05-03: `GET /api/admin/support/inbox` e `/admin/support` exibem eventos operacionais redigidos de grants, acessos protegidos e acoes da conta demo; falta ligar WhatsApp oficial MAYUS. Usuario confirmou que o SQL de `admin_support_grants` foi aplicado no Supabase. Validacoes passaram: 13 testes focados e typecheck.

## Proximo Passo

- [x] Criar UI/painel super admin para executar dry-run/reset da conta demo, marcar o tenant como demo e checar prontidao do Drive dedicado.
- [x] Adicionar banner visual permanente para impedir confusao entre conta modelo e tenant real.
- [ ] Implementar contrato/tipos e flags para `demo_mode`, `mayus_support_admin` e contas WhatsApp por dono operacional.
- [x] Criar grants temporarios de suporte MAYUS com motivo, escopo, expiracao e auditoria.
- [x] Criar primeira rota de suporte protegida por grant ativo, sem expor dados brutos ou segredos.
- [~] Criar inbox de suporte MAYUS separada dos tenants, com eventos redigidos.
