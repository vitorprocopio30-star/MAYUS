...
| Financeiro | 67% | Financeiro Beta separa receita SaaS do MAYUS em `platform_billing_events`, entrega painel superadmin com MRR/ARR/receita/risco por escritorio, resumo financeiro do tenant, `collections_followup`, labels financeiros no Chat/Brain, reconciliacao leve, forecast por prazo, aging de vencidos e riscos por cliente/caso. A aba Financeiro do escritorio passou smoke autenticado com fixture controlada; ainda faltam smoke real Asaas, migration remota do billing SaaS, superadmin E2E e forecast completo por funil/contrato. Dry-run remoto esta bloqueado nesta sessao porque o Supabase CLI nao tem `SUPABASE_ACCESS_TOKEN`. |
...
- [~] Visibilidade financeira no Chat/Brain: respostas do MAYUS destacam `asaas_billing`, `collections_followup_plan` e revenue-to-case quando o kernel retorna essas capabilities/artifacts; painel superadmin, summary do tenant, forecast por prazo, aging de vencidos e riscos por cliente/caso ja estao implementados. A aba Financeiro do dashboard passou smoke autenticado com fixture controlada; falta smoke visual do `/admin` apos aplicar migration remota de billing SaaS e usar usuario E2E superadmin.
...
- [~] Criar forecast por funil, proposta, contrato, cobranca e inadimplencia: summary financeiro ja separa cobrancas a vencer em 7 dias, 30 dias, futuro e sem vencimento e passou smoke autenticado no dashboard; falta forecast completo por funil/proposta/contrato.
