...
| Financeiro | 66% | Financeiro Beta separa receita SaaS do MAYUS em `platform_billing_events`, entrega painel superadmin com MRR/ARR/receita/risco por escritorio, resumo financeiro do tenant, `collections_followup`, labels financeiros no Chat/Brain, reconciliacao leve, forecast por prazo, aging de vencidos e riscos por cliente/caso em codigo/teste. Ainda faltam smoke real Asaas, migration remota, forecast completo por funil/contrato e validacao visual autenticada. |
...
- [~] Visibilidade financeira no Chat/Brain: respostas do MAYUS destacam `asaas_billing`, `collections_followup_plan` e revenue-to-case quando o kernel retorna essas capabilities/artifacts; painel superadmin, summary do tenant, forecast por prazo, aging de vencidos e riscos por cliente/caso ja estao implementados em codigo/teste, faltando smoke visual autenticado e dados reais/controlados.
...
- [~] Reconciliacao leve `financials` + Brain artifacts + `process_tasks` existe em helper testado, com status `matched`/`partial`/`blocked`/`unmatched`; ja esta plugada no resumo financeiro do tenant em codigo/teste, faltando smoke real com dados controlados antes de marcar como operacional.
- [~] Criar forecast por funil, proposta, contrato, cobranca e inadimplencia: summary financeiro ja separa cobrancas a vencer em 7 dias, 30 dias, futuro e sem vencimento; falta forecast completo por funil/proposta/contrato e smoke autenticado.
- [~] Mostrar risco financeiro por tenant e por cliente: superadmin ja mostra risco por escritorio/tenant, e o summary do tenant agora lista riscos por cliente/caso agrupados a partir de `financials`; falta smoke visual autenticado com dados reais/controlados.
