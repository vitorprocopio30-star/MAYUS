...
| --- | ---: | --- |
| MAYUS geral | 81% | Produto forte, WhatsApp multimodal, bloqueio de grupos, recuperacao de locks, observabilidade, fila de resposta, ACK de midia, politica de qualidade agentica, Drive Scanner supervisionado, Document Evidence Pack e Peças Verificáveis evoluiram, mas ainda nao e o socio virtual completo. |
| Produto juridico/base SaaS | 81% | Dashboard, CRM, documentos, juridico, agenda, marketing, permissoes, cockpit beta do Drive Scanner, Evidence Pack documental e guardrails de Peças Verificáveis ja existem. |
...
| Juridico/Lex | 90% | Base juridica e documental esta forte; beta de revisao humana de movimentacoes, classificador com polo/obrigacao/motivo de confianca, Kanban/prazo seguro, badges de origem, Drive Scanner cockpit, Document Evidence Pack e Peças Verificáveis ja foram mergeados; faltam cronologia persistente, detector documental profundo e smoke real volumoso. |
...
| Integracoes e operacao real | 78% | WhatsApp Evolution tem smoke, observabilidade, alerta de falha, job assincrono validado manualmente, ACK de midia deployado, bloqueio de grupos, filas zeradas, audio de comando interno tratado antes do ACK, autoenvio agentico endurecido e Drive Scanner com apply/revert supervisionado. Faltam smoke privado fechado com audio real do dono, PDF novo, Meta Cloud, scheduler automatico observado e Drive legado real com volume. |
...
- [~] Drive Scanner beta em Documentos com preview, fila humana, apply supervisionado e revert seguro; falta validar com Drive legado real/volume de escritorio.
- [~] Document Evidence Pack beta em Documentos com preview, gaps, riscos, base factual citavel e snapshot auditavel em `brain_artifacts`; falta smoke real com acervo juridico volumoso.
- [~] Peças Verificáveis: geracao, aprovacao/publicacao e export oficial agora exigem Evidence Pack atual pronto, snapshot verificavel e fonte factual auditavel; o refresh de snapshot verificavel sem alterar texto existe para drafts atuais. Falta smoke real com acervo juridico volumoso.
...
- [~] Movimentacoes processuais agora passam por politica segura: alta confianca deterministica pode criar prazo/card com auditoria; media/baixa confianca, arquivamento e extincao vao para revisao humana.
- [~] Fila beta de revisao juridica em `/dashboard/aprovacoes` lista pendentes, permite editar acao/data/nota, aprovar/ignorar e recuperar revisoes travadas; falta trocar `system_event_logs` por tabela propria transacional.
- [~] Aprovacao humana cria/vincula card Kanban e prazo com reserva atomica, rollback para revisao em falha e deduplicacao de timeline; aprovacoes de arquivamento/extincao seguem bloqueadas manualmente no beta.
- [~] Tela de prazos mostra origem (`Revisado por humano`, `IA alta confianca`, `Manual`) e alerta `Sem card` para prazo sem `process_task_id`; falta validar com volume real de escritorio.
...
- [~] Classificador de movimentacoes evoluido com `polo_representado`, `obrigacao_de_quem`, `confidence_reason` e `review_required` explicito antes de aumentar automacao; falta validar volume real de escritorios e trocar workflow state para tabela propria.
- [~] Document Evidence Pack mergeado no PR #11: builder monta memoria documental, documentos, base factual, gaps, riscos e checklist de citacao; API `GET/POST /api/documentos/processos/[taskId]/evidence-pack` permite preview sem persistir e snapshot auditavel `document_evidence_pack`; UI em `/dashboard/documentos` mostra fontes, lacunas e riscos. O pack ja e usado como trava nas Peças Verificáveis; falta validar com acervo real volumoso.
- [~] Peças Verificáveis mergeadas no PR #13: geracao usa `factBasis` quando o Evidence Pack atual esta pronto, Draft Factory persiste `verified_piece`, `evidence_pack_artifact_id`, `fact_basis` e `pending_validations`, aprovacao/publicacao exige snapshot verificavel atual, export oficial bloqueia rascunho sem versao formal verificavel, e `publishLegalPiecePremium` valida processo/versao antes de publicar. O fluxo UX/API para atualizar snapshot verificavel sem alterar texto foi iniciado para drafts atuais; falta smoke real com acervo volumoso.
...
- [~] Drive Scanner beta tem cockpit em `/dashboard/documentos` com historico de scans, fila humana, badges de risco, aprovacao/rejeicao, apply supervisionado e revert de acoes aplicadas; falta smoke real com Drive legado volumoso.
- [~] Evidence Pack documental aponta gaps, documentos sem texto, duplicados provaveis, arquivos fora da estrutura, memoria stale e baixa cobertura documental, e agora alimenta Peças Verificáveis como trava factual; contradicoes/documentos antigos ainda exigem detector documental profundo e validacao real.
- [ ] Criar cronologia documental.
- [~] Organizar Drive legado com fila auditavel de revisao e revert seguro: fluxo beta existe, mas ainda precisa validacao real controlada antes de marcar `[x]`.
...
- [~] Smoke Playwright juridico cobre `/dashboard/aprovacoes` e `/dashboard/operacoes/prazos` com mocks de revisao/recovery/badges; falta smoke real controlado com dados de escritorio.
...
- [~] Capability `legal_process_mission_execute_next` adicionada: o router, registry, prompt de chat e dispatcher agora permitem executar o proximo passo seguro de uma missao processual. Nesta fase, `refresh_document_memory` roda automaticamente via `legal_document_memory_refresh`; baixa confianca bloqueia a missao; e `generate_first_draft` nao chama a Draft Factory diretamente, abrindo approval humano supervisionado para `legal_first_draft_generate` com risco alto, processo, peca sugerida, objetivo, fontes e lacunas. O handler registra artifact `process_mission_step_result` e learning event `process_mission_step_executed`. Validado em 2026-05-12 com suite focada Lex/Brain (`npx.cmd vitest run "src/lib/lex/process-mission-context.test.ts" "src/lib/lex/case-context.test.ts" "src/lib/agent/kernel/router.test.ts" "src/lib/agent/capabilities/dispatcher.test.ts" "src/app/api/ai/chat/route.test.ts" "src/app/api/ai/approve/route.test.ts" "src/lib/brain/turn.test.ts" "src/app/api/brain/chat-turn/route.test.ts"`, 8 arquivos, 82 testes), `npx.cmd tsc --noEmit --pretty false` e `npm.cmd run build`. Smoke autenticado em producao passou em 2026-05-15 no deploy `dpl_8Yu7UMpLorqwv2Sh24eS6o5V1yv7` do merge `2f7a9dcc`: `legal_process_mission_execute_next` resolveu `processo E2E-2026-0001`, criou artifact `8c94adb5-19e4-49ef-99fc-be31e924c5a0` e event `4fe84acc-4758-4dc8-9498-83175496dd89`, falhou com seguranca em `legal_document_memory_refresh` por `GoogleDriveSyncFailed: Bad Request` e manteve `external_side_effects_blocked: true`.
- [~] Execucao deterministica da missao processual no chat e no Brain adicionada: `/api/ai/chat` agora executa diretamente `legal_process_mission_plan` e `legal_process_mission_execute_next` quando o router local identifica alta confianca sem ambiguidade, antes de depender do LLM chamar ferramenta. O router tambem passou a aceitar `proximo/proxima/acao` com acento e referencia textual como `processo da Maria da Silva`; o resolvedor juridico agora trata `process_task_id` nao-UUID como referencia operacional para evitar busca por ID invalido quando o LLM envia `E2E-2026-0001` no campo errado. `executeBrainTurn` propaga `taskId/runId/stepId`, atualiza `brain_tasks`/`brain_runs`/`brain_steps`, registra learning event, artifact `mission_result` quando conclui e `brain_approvals` quando o kernel retorna `awaiting_approval`. Validado em 2026-05-12 pela mesma suite focada Lex/Brain (82 testes), typecheck e build; smoke autenticado em producao passou em 2026-05-15 apos PR #4.
...
- [~] Case Brain 2.0 inicial implementado: nova capability `legal_case_brain_insights` gera cronologia estruturada, mapa de riscos, contradicoes/divergencias, fatos documentados, inferencias, hipoteses, proximos atos provaveis e lacunas de grounding sem side effects externos. O router, registry, prompt do chat, dispatcher e highlight do MAYUS foram conectados; a execucao cria artifact `legal_case_brain_insights` e learning event `legal_case_brain_insights_created`. Atualizacao: o dispatcher agora carrega evidencias reais tolerantes a falha de `process_documents`, `process_document_contents`, `process_movimentacoes` e `process_movimentacoes_inbox`, usando documentos extraidos, documentos sem extracao, arquivos fora da estrutura e movimentacoes que exigem acao para enriquecer cronologia, riscos, fatos e proximos atos. Atualizacao operacional: `legal_process_mission_execute_next` passou a consultar o Case Brain 2.0 antes de executar, bloqueando contradicao critica, movimentacao que exige acao e risco alto nao compativel com simples refresh documental; aprovacoes de minuta carregam contadores de risco/contradicao do Case Brain. Validado com `npm.cmd test -- src/lib/lex/case-brain-insights.test.ts src/lib/agent/capabilities/dispatcher.test.ts src/lib/agent/kernel/router.test.ts src/app/api/ai/chat/route.test.ts` (80 testes), `npm.cmd test -- src/lib/lex/case-context.test.ts src/lib/agent/kernel/router.test.ts src/lib/agent/capabilities/dispatcher.test.ts` (85 testes), `npx.cmd tsc --noEmit --pretty false` e smoke autenticado em producao no processo `E2E-2026-0001`: task `5bbd7601-7e46-43a3-93fb-3da998dd8413`, artifact `0418dd64-0184-4801-8ebe-17bd6cf4fe0f`, event `3ddcabdb-35d2-4130-a115-6dca676057eb`, `timeline_count: 7`, `risk_count: 1`, `contradiction_count: 1`, `evidence_document_count: 1`, `external_side_effects_blocked: true`. Ainda falta persistencia propria da cronologia e detector documental profundo antes de marcar como `[x]`.
- [~] Beta de movimentacoes processuais seguras mergeado no PR #6 (`legal-movement-review-beta`): `analisador.ts` diferencia alta confianca deterministica de casos que exigem revisao humana, webhook Escavador respeita `paid_summary_recommended`, API `/api/juridico/movement-reviews` permite listar/aprovar/ignorar/recuperar revisoes, UI `/dashboard/aprovacoes` ganhou fila de revisao juridica e recovery, e `/dashboard/operacoes/prazos` mostra origem do prazo e alerta `Sem card`. Validado antes do merge com `npx vitest run "src/app/api/juridico/movement-reviews/route.test.ts" "src/lib/juridico/analisador.test.ts" "src/lib/juridico/process-pipeline-resolver.test.ts"` (30 testes), `npx.cmd tsc --noEmit --pretty false`, smoke Playwright `e2e/legal-movement-review-smoke.spec.ts` em porta 3100, `git diff --cached --check`, checks Vercel pos-merge e alias `https://mayus-premium-pro.vercel.app` respondendo `MAYUS | Sistema Jurídico Premium`. Ainda falta tabela propria para workflow de revisao.
- [~] Classificador juridico de movimentacoes enriquecido no PR #8 (`legal-movement-classifier`): `src/lib/juridico/analisador.ts` agora retorna `polo_representado`, `obrigacao_de_quem`, `confidence_reason` e `review_required`, persiste metadados em `analise_json`/payloads de auditoria e corrige a regra de obrigacao para priorizar a acao pratica do escritorio antes de classificar mencao a parte contraria. Validado com `npx vitest run "src/lib/juridico/analisador.test.ts" "src/app/api/juridico/movement-reviews/route.test.ts" "src/lib/juridico/process-pipeline-resolver.test.ts"` (31 testes), `npx.cmd tsc --noEmit --pretty false`, `git diff --check` e checks Vercel pos-merge. Ainda falta validar volume real antes de aumentar automacao.
- [~] Drive Scanner UI beta mergeado no PR #9 (`legal-drive-review-ui`): `/dashboard/documentos` ganhou cockpit com botao de analise, historico de scans, detalhe do run, fila humana, badges de duplicado/sem processo/confianca/falha/status, aprovar/rejeitar, aplicar acoes seguras/aprovadas e reverter aplicadas. Operacoes com side effect real no Google Drive exigem confirmacao e a UI bloqueia aprovacao de acoes sem destino seguro. Validado com `npx.cmd tsc --noEmit --pretty false`, lint focado, `npx.cmd vitest run src/lib/services/drive-document-scanner.test.ts src/app/api/documentos/drive-scan/preview/route.test.ts src/app/api/documentos/drive-scan/review/route.test.ts src/app/api/documentos/drive-scan/apply/route.test.ts src/app/api/documentos/drive-scan/revert/route.test.ts` (23 testes), smoke Playwright `e2e/documentos-authenticated.spec.ts -g "Scanner do Drive beta"` em porta 3100, `git diff --check` e checks Vercel pos-merge em `mayus`, `mayus-premium`, `mayus-premium-pro` e `mayus-v9`. Ainda falta smoke real com Drive legado volumoso e Document Evidence Pack.
- [~] Document Evidence Pack mergeado no PR #11 (`legal-document-evidence-pack`): `src/lib/services/document-evidence-pack.ts` calcula pacote verificavel com memoria, documentos, base factual, gaps, riscos e checklist de citacao; rota `GET/POST /api/documentos/processos/[taskId]/evidence-pack` permite preview e persistencia auditavel como artifact `document_evidence_pack`; `/dashboard/documentos` ganhou painel de preview/snapshot e invalida o pack quando estrutura, sync, organizacao ou upload alteram o acervo. Validado com `npx.cmd vitest run src/lib/services/document-evidence-pack.test.ts src/app/api/documentos/processos/[taskId]/evidence-pack/route.test.ts` (11 testes), `npx.cmd tsc --noEmit --pretty false`, `npm run lint` com warnings antigos, `git diff --check` e checks Vercel em `mayus`, `mayus-premium`, `mayus-premium-pro` e `mayus-v9`. Smoke Playwright local do painel foi adicionado, mas nao rodou neste worktree temporario por falta de env Supabase local; falta smoke real com acervo juridico volumoso.
- [~] Peças Verificáveis mergeadas no PR #13 (`legal-verified-pieces`, merge `ab52b2d`): `src/lib/juridico/verified-piece.ts` centraliza readiness e guardrails; a geracao de peças carrega o Evidence Pack atual e restringe planner/writer ao `factBasis` quando pronto; Draft Factory persiste snapshot `verified_piece`, `evidence_pack_artifact_id`, `fact_basis` e `pending_validations`; aprovacao/publicacao formal exigem Evidence Pack atual pronto e snapshot verificavel; export oficial exige `versionId` formal verificavel; e `publishLegalPiecePremium` bloqueia versao de outro processo. A UI de Documentos mostra status de Peça Verificável e bloqueia aprovacao/publicacao/export quando a versao nao esta verificavel. Validado com `npx.cmd vitest run src/lib/juridico/publish-piece-premium.test.ts src/lib/juridico/verified-piece.test.ts src/lib/lex/draft-versions.test.ts src/app/api/documentos/processos/[taskId]/exportar-peca/route.test.ts src/app/api/documentos/processos/[taskId]/minutas/[versionId]/route.test.ts` (5 arquivos, 30 testes), `npx.cmd tsc --noEmit --pretty false`, `npm test` (113 arquivos, 620 testes), `npm run lint` com warnings antigos, `git diff --check` sem erro real e checks Vercel em `mayus`, `mayus-premium`, `mayus-premium-pro` e `mayus-v9`. Ainda falta smoke real com acervo juridico volumoso.
- [~] Refresh de snapshot de Peça Verificável iniciado: `PATCH /api/documentos/processos/[taskId]/minutas/[versionId]` aceita `refresh_verification`, reavalia o Evidence Pack atual e atualiza somente metadados verificaveis da versao draft atual (`verified_piece`, `evidence_pack_artifact_id`, `fact_basis`, `pending_validations`, `warnings`, `requires_human_review`), sem alterar `draft_markdown` nem avancar workflow. A UI mostra o botao `Atualizar snapshot sem alterar texto` quando a draft atual esta bloqueada por verificacao e sem edicoes nao salvas. Validado com testes focados de `draft-versions` e da rota de minuta (`22 passed`), `tsc`, `npm run lint` com warnings antigos e `git diff --check` sem erro real. Ainda falta smoke real com acervo volumoso.
...
- [~] Revisao juridica beta tem auditoria em `system_event_logs` (`legal_movement_review_required`, `legal_movement_review_approved`, `legal_movement_review_ignored`, `legal_movement_review_recovered`, `legal_movement_auto_create_failed`), mas ainda usa log como estado operacional; ideal e migrar para tabela propria antes de escala.
