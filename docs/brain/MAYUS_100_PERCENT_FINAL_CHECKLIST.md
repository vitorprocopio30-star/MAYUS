...
- [~] Webhook Evolution confirma recebimento de imagem/documento/PDF em segundos com ACK deterministico e tenta processar a midia por `messageId` com timeout curto; deploy concluido, falta smoke privado fechado com PDF/contracheque novo.
...
- [~] Tirar preparacao pesada de resposta de texto do webhook e enfileirar para processamento interno protegido, com disparo imediato por `message_id`, timeout curto, fallback juridico seguro e recuperacao de locks `processing`; falta smoke privado fechado apos o ajuste.
...
- [~] Smoke privado real com contracheque/PDF passou no ACK imediato e processamento de midia, mas a resposta curta `O credcesta` ficou sem retorno porque o Operating Partner expirou e o fallback deterministico bloqueava autoenvio; runtime agora classifica `Credcesta` como `payroll_discount`, permite fallback deterministico em timeout do Operating Partner para topicos juridicos seguros (`payroll_discount`/`benefit_or_inss`) e registra `blocked_reason` quando `can_auto_send = false`. Validado com `npx.cmd vitest run src/lib/growth/whatsapp-sales-reply.test.ts src/lib/growth/whatsapp-sales-reply-runtime.test.ts` (14 testes), `npx.cmd tsc --noEmit --pretty false`, deploy `dpl_99HK5huZegFjgGNYg9LwmBfVKKme` do commit `3cf9720` em `READY/PROMOTED`, e filas pos-deploy zeradas; falta repetir `O credcesta`.
