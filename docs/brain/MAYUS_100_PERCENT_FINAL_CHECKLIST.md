...
- [~] Monitoramento + WhatsApp Operating Partner atualizado em 2026-05-27: schema remoto Brain e leases/RPC de `process_update_queue` foram aplicados; worker de processos validado com `claim_source = "rpc"` e fila zerada em producao. A frente WhatsApp agora aparece como superficie da Frente B Agentic Core, a auditoria `/api/whatsapp/agent-audit` devolve health/fila/governanca e permite `Processar pendentes agora` para executivo do tenant, com painel visivel em `/dashboard/conversas/whatsapp`. Mantem `[~]` ate concluir smoke real Evolution controlado com texto, status por nome/CNJ, audio, imagem/documento, cobranca/contrato sensivel e duas mensagens em sequencia.
