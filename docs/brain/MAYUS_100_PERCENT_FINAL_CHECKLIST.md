...
- [~] Smoke privado real com contracheque/PDF passou no ACK imediato e processamento de midia, mas a resposta curta `O credcesta` ficou sem retorno porque o Operating Partner expirou e o fallback deterministico bloqueava autoenvio; runtime agora classifica `Credcesta` como `payroll_discount`, permite fallback deterministico em timeout do Operating Partner para topicos juridicos seguros (`payroll_discount`/`benefit_or_inss`) e registra `blocked_reason` quando `can_auto_send = false`. Validado com `npx.cmd vitest run src/lib/growth/whatsapp-sales-reply.test.ts src/lib/growth/whatsapp-sales-reply-runtime.test.ts` (14 testes) e `npx.cmd tsc --noEmit --pretty false`; falta deploy e repetir `O credcesta`.
