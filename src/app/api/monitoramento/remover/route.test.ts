...
function buildRequest(body: Record<string, unknown>) {
  return new NextRequest("http://localhost:3000/api/monitoramento/remover", {
    method: "DELETE",
...
describe("DELETE /api/monitoramento/remover", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("bloqueia remocao local quando cancelamento externo falha", async () => {
    ...
    const response = await DELETE(buildRequest({ processo_id: "0001" }));
