...
async function run(request: NextRequest) {
  ...
  try {
    const result = await processPendingWhatsAppRepliesBatch({
    ...
    console.error("[whatsapp-replies-process] fatal", error);
    return NextResponse.json(
      { error: error?.message || "Erro ao processar respostas WhatsApp." },
      { status: 500 },
    );
  }
}
