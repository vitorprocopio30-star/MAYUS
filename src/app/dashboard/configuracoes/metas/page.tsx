  ...
  return (
    <div className={`flex-1 overflow-auto bg-white dark:bg-[#050505] min-h-screen text-gray-900 dark:text-white p-6 sm:p-10 hide-scrollbar ${montserrat.className}`}>
      <div className="max-w-4xl mx-auto">
        ...
        {/* BREADCRUMB */}
        <Link href="/dashboard/configuracoes" className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2 hover:text-gray-900 dark:text-white transition-colors mb-6 w-max">
        ...
        {(isCreating || editingId) && (
          <div className="bg-gradient-to-br from-[#111] to-[#0a0a0a] border border-[#10B981]/20 p-6 rounded-2xl shadow-xl mb-8 animate-in slide-in-from-top-4 duration-500">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={resetForm} className="text-gray-500 hover:text-gray-900 dark:text-white transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-100 dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-lg px-4 py-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#10B981]/50 placeholder:text-gray-600 transition-colors"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className="w-full bg-gray-100 dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-lg px-4 py-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#10B981]/50 transition-colors cursor-pointer"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
               <div>
                  ...
                  <div className="flex gap-2">
                     <button onClick={() => setDraftType('CONTRATOS')} className={`flex-1 py-3 rounded-lg border font-bold text-xs flex items-center justify-center gap-2 transition-all ${draftType === 'CONTRATOS' ? 'bg-[#10B981]/10 border-[#10B981] text-[#10B981]' : 'bg-gray-100 dark:bg-[#111] border-gray-200 dark:border-white/10 text-gray-500'}`}>
                        <FileText size={16} /> Por Contratos
                     </button>
                     <button onClick={() => setDraftType('VALOR')} className={`flex-1 py-3 rounded-lg border font-bold text-xs flex items-center justify-center gap-2 transition-all ${draftType === 'VALOR' ? 'bg-[#10B981]/10 border-[#10B981] text-[#10B981]' : 'bg-gray-100 dark:bg-[#111] border-gray-200 dark:border-white/10 text-gray-500'}`}>
               ...
               <div>
                 ...
                 <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full bg-gray-100 dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-lg py-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#10B981]/50 placeholder:text-gray-600 font-mono transition-colors ${draftType === 'VALOR' ? 'pl-10 pr-4' : 'px-4'}`}
                    />
                 </div>
               </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-white/5">
              <button onClick={resetForm} className="text-xs font-bold px-4 py-2 text-gray-400 hover:text-gray-900 dark:text-white">
                Cancelar
              </button>
              <button
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="bg-gradient-to-br from-[#111] to-[#0a0a0a] border border-gray-200 dark:border-white/5 p-10 rounded-2xl shadow-xl text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {goals.map((goal) => (
                 <div key={goal.id} className="group relative bg-[#0C0C0C] border border-gray-200 dark:border-white/5 p-6 rounded-2xl hover:border-[#10B981]/30 transition-all overflow-hidden flex flex-col justify-between">
                    ...
                    <div>
                       <div className="flex items-start justify-between mb-3 relative z-10">
                          <h4 className="font-bold text-gray-900 dark:text-white text-lg pr-4 line-clamp-2 leading-tight">{goal.name}</h4>
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity absolute right-0 top-0 bg-[#0C0C0C] pl-2">
                             <button onClick={() => startEdit(goal)} className="p-1.5 bg-gray-100 dark:bg-white/5 hover:bg-gray-100 dark:bg-white/10 rounded-md text-gray-400 hover:text-gray-900 dark:text-white transition-colors"><Edit3 size={14} /></button>
                             <button onClick={() => handleDelete(goal)} className="p-1.5 bg-gray-100 dark:bg-white/5 hover:bg-red-500/20 rounded-md text-gray-400 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
                          </div>
                       </div>
                       <div className="flex gap-2 mb-6">
                          ...
                          <span className="text-[9px] bg-gray-100 dark:bg-white/5 text-gray-400 px-2 py-0.5 rounded border border-gray-200 dark:border-white/10 uppercase tracking-widest font-black">
                             {goal.period}
                          </span>
                       </div>
                    </div>
                    ...
                    <div className="mt-auto">
                       ...
                       <p className="text-2xl font-black text-gray-900 dark:text-white font-mono tracking-tight">{formatValue(goal.target_value, goal.type)}</p>
                    </div>
                 </div>
               ))}
            </div>
