...
export default function UsuariosPermissoesPage() {
  ...
  const handleAvatarUpload = async (member: TeamMember, file: File | null) => {
    if (!file) return;
    if (!tenantId) {
      toast.error("Tenant nao identificado para upload de avatar.");
      return;
    }
    ...
    setUploadingAvatarId(member.id);
    try {
      const optimizedFile = await optimizeAvatarForUpload(file);
      const filePath = `${tenantId}/${member.id}/avatar.jpg`;
      ...
      if (!publicUrl) {
        throw new Error("URL publica nao retornada para avatar.");
      }
      ...
      if (!response.ok) {
        throw new Error(result?.error || "Avatar enviado, mas nao foi possivel salvar no perfil.");
      }

      setMembers((prev) => prev.map((item) => item.id === member.id ? { ...item, avatar_url: publicUrl } : item));
      setBrokenAvatars((prev) => ({ ...prev, [member.id]: false }));
      ...
      if (message.toLowerCase().includes("maximum allowed size") || message.toLowerCase().includes("object exceeded")) {
        ...
        toast.error("Bucket de avatars indisponivel ou sem permissao de upload.");
      } else {
        toast.error(message || "Falha ao atualizar avatar.");
      }
    } finally {
  ...
  return (
    <div className={`space-y-8 ${montserrat.className}`}>
              ...
              <div key={member.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-4 px-6 py-5 border-b border-gray-200 dark:border-white/5 hover:bg-gray-50 dark:bg-white/[0.02] transition-colors items-center">
                <div className="col-span-4 flex items-center gap-3">
                  <label className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold shrink-0 overflow-hidden relative cursor-pointer group/avatar ${member.is_active ? "bg-gradient-to-tr from-[#CCA761] to-[#8B7340] text-black" : "bg-gray-800 text-gray-500"}`}>
                    ...
                    <span className="absolute inset-0 hidden items-center justify-center bg-black/60 text-white group-hover/avatar:flex">
                      {uploadingAvatarId === member.id ? <Loader2 size={14} className="animate-spin text-[#CCA761]" /> : <Camera size={14} />}
                    </span>
                    {uploadingAvatarId === member.id && (
                      <span className="absolute inset-0 flex items-center justify-center bg-black/70">
                        <Loader2 size={14} className="animate-spin text-[#CCA761]" />
                      </span>
                    )}
                  </label>
      ...
      {(showInviteModal || editMember) && (
        <div className="fixed inset-0 bg-gray-200 dark:bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
