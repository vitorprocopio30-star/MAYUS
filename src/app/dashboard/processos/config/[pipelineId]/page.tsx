...
export default function ConfigProcessoPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        toast.error("Erro ao carregar Configurações do Processo.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSave = async () => {
    ...
    if (!name.trim()) { toast.error("O nome do Processo é obrigatório."); return; }

    setIsSaving(true);
    try {
      // 1. Update pipeline
      const { error: pipelineError } = await supabase.from("process_pipelines").update({
        name,
        description,
        tags: pipelineTags,
      ...
      // 2. Sync stages
      const { data: existingStages } = await supabase.from("process_stages").select("id").eq("pipeline_id", pipelineId);
      const existingIds = existingStages?.map((s: any) => s.id) || [];
      const currentIds = stages.filter(s => !s.is_new).map(s => s.id);
      
      const idsToDelete = existingIds.filter(id => !currentIds.includes(id));
      if (idsToDelete.length > 0) {
        await supabase.from("process_stages").delete().in("id", idsToDelete);
      }
  ...
  const handleDeletePipeline = async () => {
    if (!window.confirm("Certeza absoluta? Todas as etapas e tarefas deste Processo serão deletadas para sempre.")) return;
    
    setIsSaving(true);
    try {
      const { error } = await supabase.from("process_pipelines").delete().eq("id", pipelineId);
      ...
      <header className="flex-none bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5 sticky top-0 z-30">
        <div className="px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link 
              href={`/dashboard/processos/${pipelineId}`}
        ...
        <div className="max-w-4xl space-y-8">
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-300">Nome do Processo</label>
              <input 
                type="text" 
                value={name}
                ...
                <Droppable droppableId="stages-list">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-3 w-full md:w-auto mt-2 md:mt-0 pt-3 md:pt-0 border-t border-white/5 md:border-t-0 pl-2 z-10">
                    ...
                    <button 
                      ...
                      className="p-2.5 text-gray-500 hover:text-red-400 hover:bg-black/40 border border-transparent hover:border-red-500/20 rounded-xl transition-colors ml-auto md:ml-2 shadow-sm"
                      title="Excluir etapa"
                    >
                      <Trash2 size={16} />
                    </button>
          ...
          {/* SEC 3: Etiquetas do Processo */}
          <section className="space-y-4 pt-6 mt-6 border-t border-white/5">
            <h2 className="text-lg font-bold text-white mb-2">Etiquetas</h2>
            <div className="bg-[#111] border border-white/5 rounded-xl p-4">
              <p className="text-sm text-gray-400 mb-4">Crie etiquetas personalizadas que estarão disponíveis para os leads deste Processo.</p>
              ...
              <div className="flex gap-3 mb-6">
                <input 
                  ...
                  className="flex-1 bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#CCA761]/50 transition-colors"
          ...
          {/* SEC 3.5: Setores do Processo */}
          <section className="space-y-4 pt-6 mt-6 border-t border-white/5">
            <h2 className="text-lg font-bold text-white mb-2">Setores (Transferência)</h2>
            <div className="bg-[#111] border border-white/5 rounded-xl p-4">
              <p className="text-sm text-gray-400 mb-4">Cadastre os setores para os quais você deseja atribuir/transferir as oportunidades dentro deste Processo.</p>
              ...
              <div className="flex gap-3 mb-6">
                <input 
                  ...
                  className="flex-1 bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-colors"
          ...
          <section className="space-y-4 pt-8 mt-8 border-t border-white/5 relative">
            ...
            <div className="bg-gradient-to-br from-[#111] to-[#0a0a0a] border border-white/5 rounded-2xl p-6 shadow-[0_8px_30px_rgba(0,0,0,0.5)] relative overflow-hidden group">
              ...
              <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                Adicione os canais de recebimento (ex: WhatsApp, Instagram) que estarão vinculados a este Processo. Estes canais serão acionados automagicamente.
              </p>
              ...
              <div className="flex items-center gap-3 mb-6 relative">
                <input 
                  ...
                  className="flex-1 bg-black/40 border border-[#2a2a2a] rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#CCA761]/50 focus:ring-1 focus:ring-[#CCA761]/30 transition-all placeholder:text-gray-600"
                ...
                {pipelineInboxes.map(inbox => (
                  <div key={inbox} className="inline-flex items-center gap-3 bg-gradient-to-r from-[#222] to-[#1a1a1a] text-gray-200 text-xs font-bold px-4 py-2 rounded-xl border border-white/10 shadow-lg group/item transition-all hover:bg-white/5">
                    ...
                    <button 
                      ...
                      className="ml-2 text-gray-500 hover:text-red-400 p-1 hover:bg-black/50 rounded-md transition-colors"
          ...
          <section className="space-y-6 pt-8 mt-8 border-t border-white/5">
            <div>
              <h2 className="text-xl font-bold text-white tracking-wide">Automações do Processo</h2>
          ...
          {/* SEC 5: Excluir Processo */}
          <section className="space-y-4 pt-8 mt-6 border-t border-white/5">
            <div>
              <h2 className="text-lg font-bold text-red-400 mb-1">Excluir Processo</h2>
              <p className="text-sm text-gray-500">Depois de excluir um Processo, não há como voltar atrás. Todas as tarefas e etapas serão excluídas permanentemente.</p>
            </div>
            <button 
            ...
            >
              <Trash2 size={16} /> Excluir Processo
            </button>
          </section>
