  ...
  return (
    <div className={`space-y-8 ${montserrat.className}`}>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 flex items-center gap-3 bg-gray-100 dark:bg-[#111111] px-4 py-3 rounded-xl border border-[#222] focus-within:border-[#CCA761]/50 transition-colors">
          ...
          <input
            ...
            className="bg-transparent border-none outline-none text-sm w-full text-gray-800 dark:text-gray-200 placeholder:text-gray-600"
          />
        </div>
        <div className="flex bg-gray-100 dark:bg-[#111111] border border-[#222] rounded-xl p-1 shrink-0">
          <button
            onClick={() => setFilterType("ALL")}
            className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all ${filterType === "ALL" ? "bg-[#CCA761]/10 text-[#CCA761]" : "text-gray-500 hover:text-gray-700 dark:text-gray-300"}`}
          >
            TODOS
          </button>
          <button
            onClick={() => setFilterType("PF")}
            className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all ${filterType === "PF" ? "bg-[#CCA761]/10 text-[#CCA761]" : "text-gray-500 hover:text-gray-700 dark:text-gray-300"}`}
          >
            FÃSICA
          </button>
          <button
            onClick={() => setFilterType("PJ")}
            className={`px-4 py-2 text-xs font-semibold rounded-lg transition-all ${filterType === "PJ" ? "bg-[#CCA761]/10 text-[#CCA761]" : "text-gray-500 hover:text-gray-700 dark:text-gray-300"}`}
          >
            JURÃDICA
          </button>
        </div>
      ...
      <div className="bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 rounded-2xl backdrop-blur-sm relative z-0">
        ...
        {profileLoading || isLoading ? (
          ...
          filteredClients.map((client) => (
            <div key={client.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 px-6 py-5 border-b border-gray-200 dark:border-white/5 hover:bg-gray-50 dark:bg-white/[0.02] transition-colors items-center">
              <div className="col-span-4 flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm shrink-0 border border-gray-200 dark:border-white/10 ${client.type === 'PF' ? "bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-gray-300" : "bg-[#CCA761]/10 border-[#CCA761]/30 text-[#CCA761]"}`}>
                  {client.type === 'PF' ? <UserIcon size={18} /> : <Building2 size={18} />}
                </div>
                <div>
                  <p className="text-gray-900 dark:text-white font-medium text-sm">{client.name}</p>
                  <span className="text-gray-500 text-[10px] uppercase tracking-wider">{client.type === 'PF' ? 'Pessoa FÃ­sica' : 'Pessoa JurÃ­dica'}</span>
                </div>
              </div>

              <div className="col-span-2">
                <span className="text-gray-700 dark:text-gray-300 text-xs font-mono">{client.document}</span>
              </div>

              <div className="col-span-3">
                <p className="text-gray-700 dark:text-gray-300 text-xs mb-1 truncate">{client.email || 'â€”'}</p>
              ...
              <div className="col-span-1 flex justify-end">
                <button
                  ...
                  className="p-2 rounded-lg hover:bg-gray-100 dark:bg-white/5 text-gray-500 hover:text-[#CCA761] transition-colors flex items-center gap-1 group"
                >
